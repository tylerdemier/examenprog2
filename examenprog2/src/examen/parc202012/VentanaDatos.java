...
public class VentanaDatos extends JInternalFrame {
	 ...
	 * @param runnable Objeto runnable con código a ejecutar (run()) cuando el
	 *                 botón se pulse
	...
	public VentanaDatos(VentanaGeneral ventMadre, String tipo, String titulo) {
		...
		tDatos.addKeyListener(new KeyAdapter() {
			...
			private void hacerScrollHastaFila(int row, int col, boolean haciaAbajo) { // Mostrar en el scrollpane la
				...
				if (haciaAbajo)
					rect.setLocation(rect.x, rect.y + 25); // Y un poquito más abajo (para que no quede demasiado
															// justo)
				else
		 ...
		 * lMensaje.setText( " " ); } else { lMensaje.setText(
		 * "Valor de satisfacción: " + tDatos.getModel().getValueAt( fila, columna ) );
		 * } } else { lMensaje.setText( " " ); } } }); tDatos.addMouseListener( new
		 * MouseAdapter() {
		 * 
